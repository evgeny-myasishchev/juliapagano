...
var BootApp = function () {
  ...
  this.withControllers = defineStage(function (app, controllers) {
    app.get('/admin', routeTo(controllers, 'admin_controller', 'index'));
  });

  this.withReadModels = defineStage(function (app) {
    var db = app.get('db');
    var PagesReadModel = require('../app/models/pages-read-model').PagesReadModel;
    app.set('pages-read-model', new PagesReadModel(db));
  });

  this.withPages = defineStage(function (app, controller) {
    app.get('pages-read-model').getPages().then(function (pages) {
      app.locals.pages = pages;
      _.forEach(pages, function (page) {
        if (!page.isExternal) {
          app.get(page.path, controller.createPageAction(page));
        }
      });
    });
  });
